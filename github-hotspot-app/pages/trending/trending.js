const api = require('../../utils/api');
const util = require('../../utils/util');

Page({
  data: {
    trending: [], // 趋势仓库列表
    isLoading: false,
    hasMore: true,
    currentPage: 1,
    // 时间范围
    timeRange: 'weekly',
    // 语言选项
    languages: [
      { name: '全部', value: '' },
      { name: 'JavaScript', value: 'javascript' },
      { name: 'Python', value: 'python' },
      { name: 'Java', value: 'java' },
      { name: 'Go', value: 'go' },
      { name: 'Rust', value: 'rust' },
      { name: 'TypeScript', value: 'typescript' },
      { name: 'C++', value: 'cpp' },
      { name: 'PHP', value: 'php' },
      { name: 'Swift', value: 'swift' },
      { name: 'Kotlin', value: 'kotlin' }
    ],
    trendingLanguage: ''
  },
  
  onLoad: function() {
    this.fetchTrendingRepos();
  },
  
  onPullDownRefresh: function() {
    this.setData({
      trending: [],
      currentPage: 1,
      hasMore: true
    });
    this.fetchTrendingRepos().then(() => {
      wx.stopPullDownRefresh();
    }).catch(() => {
      wx.stopPullDownRefresh();
    });
  },
  
  onReachBottom: function() {
    if (!this.data.hasMore || this.data.isLoading) return;
    this.fetchTrendingRepos(this.data.currentPage + 1);
  },
  
  // 切换时间范围
  onTimeChange: function(e) {
    const time = e.currentTarget.dataset.time;
    this.setData({
      timeRange: time,
      trending: [],
      currentPage: 1,
      hasMore: true
    });
    this.fetchTrendingRepos();
  },
  
  // 选择语言
  onLanguageChange: function(e) {
    const language = e.currentTarget.dataset.language;
    this.setData({
      trendingLanguage: language,
      trending: [],
      currentPage: 1,
      hasMore: true
    });
    this.fetchTrendingRepos();
  },
  
  // 获取趋势仓库
  fetchTrendingRepos: function(page = 1) {
    if (this.data.isLoading) return Promise.reject();
    
    this.setData({ isLoading: true });
    
    return api.getTrendingRepos(this.data.trendingLanguage, this.data.timeRange, page)
      .then(repos => {
        // 检查仓库是否已收藏
        repos.forEach(repo => {
          repo.isFavorited = util.isFavorite(repo.id);
        });
        
        // 更新数据
        this.setData({
          trending: page === 1 ? repos : this.data.trending.concat(repos),
          isLoading: false,
          hasMore: repos.length === 10,
          currentPage: page
        });
      })
      .catch(err => {
        console.error('获取趋势仓库失败:', err);
        this.setData({ isLoading: false });
        wx.showToast({
          title: '获取数据失败',
          icon: 'none'
        });
      });
  },
  
  // 收藏状态变化处理
  onFavoriteChange: function(e) {
    const { repo, isFavorited } = e.detail;
    const index = this.data.trending.findIndex(item => item.id === repo.id);
    
    if (index > -1) {
      // 更新收藏状态
      this.setData({
        [`trending[${index}].isFavorited`]: isFavorited
      });
    }
  },
  
  // 分享功能
  onShareAppMessage: function() {
    return {
      title: 'GitHub热点 - 趋势榜',
      path: '/pages/trending/trending'
    };
  }
}); 